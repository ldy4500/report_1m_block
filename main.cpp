...
void chk_Host(const u_char* packet){
	...
	for(int i = 0; i< 300; i++){
		if(Header->http[i]== 'H'&& Header->http[i+1] == 'o'&& Header->http[i+2] == 's'&& Header->http[i+3] == 't'){
			for(int i=0;i<host_len;i++){
				if(strncmp(Header->http+i+6, host[i] , (int)strlen(host[i])-1)==0){
					allow = 1;
					break;
				}
			}
